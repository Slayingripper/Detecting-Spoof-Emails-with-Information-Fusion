...
messages = pd.read_csv(
    "/home/blackfalcon/Detecting-Spoof-Emails-with-Information-Fusion/Logistic Regression/spam.csv",
    delimiter=",\t",
    names=["label", "sms"],
)

messages["label"] = messages.label.map(
    {"ham": 0, "spam": 1}
)  # Making spam/ham as 0/1 instead
...
count_vector = CountVectorizer(
    max_features=1500
)  # Making document term matrix / frequency matrix

smsDocuments = count_vector.fit_transform(smsDocuments)

training_data, testing_data, y_train, y_test = train_test_split(
    smsDocuments, messages["label"], test_size=0.20, random_state=0
)
